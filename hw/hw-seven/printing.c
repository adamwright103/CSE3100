...
int q_fetch_job(job_queue_t *q, int id)
{
  ...
  if (r <= 0)
  {
    fprintf(stderr, "Error: printer %2d tries to get a job "
                    "after the max number of jobs have been fetched.",
            id);
    exit(EXIT_FAILURE);
  }
...
void *printer_main(void *arg)
{
  ...
  {
    // TODO
    // get the job from the queue - critical func. so use mutex
...
int main(int argc, char *argv[])
{
  ...
  for (i = 0; i < num_printers; i++)
  {
    ...
    printers[i].njobs = 0;
    if (pthread_create(&printers[i].thread_id, NULL, printer_main, &printers[i]))
    {
