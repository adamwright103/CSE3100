...
int main(int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (isdigit(argv[i][0]) && count < 3)
    {
      if (count == 0)
        nrows = atoi(argv[i]);
      else if (count == 1)
    ...
    {
      fprintf(stderr, "Usage: %s [nrows [ncols [ncols2]]] [-t]\n"
                      "<nrows> : number of rows. Default value is %d.\n"
                      "<ncols> : number of columns. Default value is %d.\n"
                      "<ncols2>: number of columns of 2nd matrix. Default value is %d.\n"
                      "-t      : show execution time.\n",
              argv[0], DEFAULT_NROWS, DEFAULT_NCOLS, DEFAULT_NCOLS2);
      return 1;
    }
  }

  if (!(nrows > 0 && ncols > 0 && ncols2))
  {
    fprintf(stderr, "All the following numbers must be positive: nrows=%d ncols=%d ncols2=%d\n", nrows, ncols, ncols2);
    exit(EXIT_FAILURE);
  }
