...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
  for (int i = 0; i < m; i++)
  {
    ...
    // check if the property is owned and pay owner
    // current player location is prop id
    ...
    else
    {
      ...
      {
        // pay person, this may fail and will return 0
        int result = transaction(&p[i], &p[prop[p[i].loc].owner_id], prop[p[i].loc].rent);
        if (result == 0)
        {
          return 0;
        }
