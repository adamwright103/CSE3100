...
node *delete_node(node *head, int v)
{
  ...
  if (it_node != NULL)
  {
    ...
    while (it_node->next != NULL)
    {
      if (it_node->next->v == v)
      {
        node *pv = it_node->next;
        it_node->next = pv->next;
        free(pv);
        return head;
      }
...
void recursive_reverse(node *n)
{
  // base case is last node of list reversed is just the last node
  // otherwise reverse by setting next to point to curr
  if (n->next == NULL)
  {
    return;
  }
  ...
  recursive_reverse(next_node);

  // set next_node to point to this node
  next_node->next = n;
  return;
}
