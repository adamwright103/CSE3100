...
void send_int(int fd, int v)
{
  if (write(fd, &v, sizeof(int)) != sizeof(int))
  {
    die("Failed to write integer to pipe.");
  }
}
...
void child_main(int fdp[], int fdc[], int seed)
{
  ...
  send_int(fdc[PIPEFD_WRITE], gmn_get_max());
  while (1)
  {
    int guess;
    if (read(fdp[PIPEFD_READ], &guess, sizeof(int)) != sizeof(int))
    {
      die("Failed to read guess from pipe.");
    }

    int result = gmn_check(&gmn, guess);
    send_int(fdc[PIPEFD_WRITE], result);
...
int main(int argc, char *argv[])
{
  ...
  read(fdc[PIPEFD_READ], &max, sizeof(int));
  ...
  do
  {
    ...
    send_int(fdp[PIPEFD_WRITE], guess);
    if (read(fdc[PIPEFD_READ], &result, sizeof(int)) != sizeof(int))
    {
      die("Failed to read result from pipe.");
    }
  ...
  char message[MSG_BUF_SIZE];
  if (read(fdc[PIPEFD_READ], message, MSG_BUF_SIZE) != MSG_BUF_SIZE)
  {
    die("Failed to read message from pipe.");
  }
