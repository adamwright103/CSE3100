...
void *thread_c(void *arg_in)
{
  ...
  while (1)
  {
    ...
    int res = gmn_check(&gmn, arg->guess);
    arg->result = res;
    if (res == 0)
...
int main(int argc, char *argv[])
{
  ...
  check_pthread_return(pthread_create(&tid_c, NULL, thread_c, &arg), "pthread_create C");
  check_pthread_return(pthread_create(&tid_p, NULL, thread_p, &arg), "pthread_create P");

  check_pthread_return(pthread_join(tid_c, NULL), "pthread_join C");
  check_pthread_return(pthread_join(tid_p, NULL), "pthread_join P");
